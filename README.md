...
Track Key는 영문 소문자, 숫자, 하이픈을 사용하는 canonical key입니다. 과거 key가 필요한 기존 DB는 Project-scoped alias로 호환하며, runtime parser는 특정 프로젝트 alias를 하드코딩하지 않습니다.
...
- 비표준 또는 등록되지 않은 `Watchtower-Track` 신호는 임의 Track으로 보정하지 않고 fail-closed 상태로 남깁니다.
- repository responsibility map의 producer class가 WatchTower 선언의 기준이며, 기술 분류 이름만으로 canonical Track을 자동 생성하지 않습니다.
...
- 특정 사용자 Project / Repository / Workflow 이름은 일반 startup 경로에서 seed하지 않습니다.
...
- Project-scoped alias가 과거 evidence를 canonical Track으로 해석
...
- Repository-scoped Project-wide/Dynamic 규칙이 다른 Repository에 전파되지 않음
- fresh install은 빈 generic registry로 시작하고 기존 DB restart는 사용자 registry를 재-seed하지 않음
- 일반 runtime source에는 특정 사용자 Project / Repository / Workflow alias 지식이 없음
